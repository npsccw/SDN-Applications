        ...
        for x in range(len(list_of_fingerprints)):
            for y in range(len(list_of_fingerprints[x])):
                ...
                if list_of_fingerprints[x][y][6]:
                    ...
                    if option55:
                        search55 = option_string.find(option55.encode('hex'))
                    if option50:
                        search50 = option_string.find(option50.encode('hex'))
                    if option12:
                        search12 = option_string.find(option12)
                ...
                if list_of_fingerprints[x][y][8]:
                    if option60:
                        search60 = list_of_fingerprints[x][y][8].find(option60)
                    if search60 != -1:
                        weight += int(list_of_fingerprints[x][y][3])   
...
def dhcp_parse(pkt):
    ...
    packet_list = [dhcp_data.xid, dhcp_data.chaddr,\
                     dhcp_data.ciaddr, dhacp_data.data\
